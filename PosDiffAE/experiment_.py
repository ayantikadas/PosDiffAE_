...
#         print("Litmodel init csv path",self.csv_path)
        ...
        self.register_buffer(
            'x_T',
            torch.randn(conf.sample_size, 3, conf.img_size_height, conf.img_size_width))
...
#         return pred_img
    ...
    def render(self, noise, cond=None, T=None,mask_mult=False,gt=None,gt_keep_mask=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
...
#         csv_path_test: str = ''
#         path: str = ''
        self.train_data,self.val_data = self.conf.make_dataset(path = self.conf.data_config_path,\
                                                csv_path =self.conf.csv_path )
#         self.train_data = SubsetDataset(self.train_data, size=100)
        
        print('train data:', len(self.train_data))
        
#         self.val_data = self.conf.make_dataset(path = self.conf.data_config_path,\
#                                                 csv_path =self.conf.csv_path_test )
#         self.train_data = SubsetDataset(self.val_data, size=100)
    ...
    def infer_whole_dataset(self,
        ...
        """
        data = self.conf.make_dataset(path=self.data_config_path,csv_path = self.csv_path)
...
#             data.transform = make_transform(self.conf.img_size, flip_prob=0)
        ...
        with writer:
            for batch in tqdm(loader, total=len(loader), desc='infer'):
                with torch.no_grad():
                    # (n, c)
                    # print('idx:', batch['index']) 'post_op_image', 'pre_op_image', 'pre_op_label', 'image_meta_dict', 'filenum', 'slicenum', 'idx'
                    cond = model.encoder(batch['_train_image'].to(self.device))
...
#                 'post_op_image', 'pre_op_image', 'pre_op_label', 'image_meta_dict', 'filenum', 'slicenum', 'idx'
                
                # print(f'(rank {self.global_rank}) batch size:', len(imgs))
                
                x_start = batch['_train_image'].to(torch.float32)
                x_start_baseline = batch['_train_image'].to(torch.float32)
                idxs = batch['idx']
                #############################
                r_value = batch['r']/batch['max_val_slice']
                r_value = r_value.to(torch.float32)
                transformation_angle = torch.rad2deg(torch.acos((torch.tensor(batch['tfm_90'])))[:,0,0])
                theta_value = (torch.rad2deg(batch['theta']).abs() + batch['add_angle'] + transformation_angle)/360
                theta_value = theta_value.to(torch.float32)
                #r = batch['transformations'][0].to(torch.float32)
                #theta = batch['transformations'][1].to(torch.float32)
                #self.sampler.r = r
                #self.sampler.theta = theta
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                self.sampler.r_value = r_value 
                self.sampler.theta_value = theta_value
                losses = self.sampler.training_losses(model=self.model,
                                                      x_start=x_start,
                                                      x_start_post=x_start_baseline,
#                                                       age_diff=age_diff,
                                                      t=t)
...
#                 grid_batch = (make_grid(batch['pre_op_image'][0:self.conf.batch_size_eval,:,:,:]) + 1) / 2
                grid_batch = (make_grid(batch['_train_image'][0:self.conf.batch_size_eval,:,:,:]) + 1) / 2
                ...
                ################# Logging conds ##############
#                 total_steps_one_epoch = self.conf.total_samples // self.conf.batch_size_effective
#                 if (self.num_samples % total_steps_one_epoch )==0:
#                 conds_save = {}
#                 conds_save["cond_pre"] = losses["cond_pre"]
#                 conds_save["cond_post"] = losses["cond_post"]
# #                 conds_save['filenum'] = batch['filenum']
#                 conds_save['slicenum'] = batch['slicenum']
# #                 f'{conf.logdir}/last.ckpt'
# #                 path__ = f'{self.conf.logdir}/z_sem'
# #                 if not os.path.exists(path__):
# #                     os.mkdir(path__)
# #                 path_save_conds = os.path.join(path__,'%d.pt' % self.trainer.current_epoch  )
# #                 torch.save(conds_save,path_save_conds)  
                ###################################################
            if self.global_rank == 0:
                ################# Logging losses ##############   
                self.logger.experiment.add_scalar('loss', losses['loss'],
                                                  self.trainer.current_epoch)
                for key in ['vae', 'latent', 'mmd', 'chamfer', 'arg_cnt','latent_code_mse','mse','sparsity_cons']:
                    if key in losses:
#                         print("losses[key]",losses[key],key)
                        self.logger.experiment.add_scalar(
                            f'loss/{key}', losses[key],  self.trainer.current_epoch)
...
#         print("self.is_last_accum(batch_idx)", self.is_last_accum(batch_idx))
        if self.is_last_accum(batch_idx):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                imgs = batch['_train_image'].to(torch.float32)
...
#                 print("self.conf.batch_size_eval",self.conf.batch_size_eval)
                ...
                for x_T in loader:
                    ...
                    if self.conf.train_mode.is_latent_diffusion(
                        ...
                        print("self.eval_sampler.sample",self.eval_sampler.sample)
            ...
            if self.conf.model_type.has_autoenc(
                ...
                if self.global_rank == 0:
                    for key, val in score.items():
                        self.logger.experiment.add_scalar(
                            f'{key}{postfix}', val,  self.trainer.current_epoch)
